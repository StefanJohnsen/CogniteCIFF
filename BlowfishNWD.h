/*----------------------------------------------------------------
  ...
  Standalone Blowfish key schedule and block cipher for encrypted
  Navisworks LcUChunk frames. LcUBlowfishFilter passes each block as
  two little-endian uint32_t words; that word order differs from the
  network-byte-order convention used by many Blowfish byte APIs.
----------------------------------------------------------------*/
...
namespace nwd::blowfish
{
    namespace detail
    {
        // Standard Blowfish initialization words (hexadecimal digits of pi).
        inline constexpr std::array<uint32_t, 18> initialP{
        ...
        inline constexpr std::array<uint32_t, 1024> initialS{
        ...
        [[nodiscard]] inline uint32_t readLittleU32(const uint8_t* bytes) noexcept
        {
        ...
        inline void writeLittleU32(uint8_t* bytes, const uint32_t value) noexcept
        {
    ...
    {
    public:
        explicit Cipher(const std::span<const uint8_t> key)
            : p_(detail::initialP), s_(detail::initialS)
        {
            if (key.empty() || key.size() > 56U)
                throw std::runtime_error("Blowfish key length must be between 1 and 56 bytes");
            ...
            for (auto& value : p_)
            {
                ...
                {
                    keyWord = (keyWord << 8U) | key[keyOffset];
                    keyOffset = (keyOffset + 1U) % key.size();
                }
                value ^= keyWord;
            }
            ...
            for (size_t index = 0; index < p_.size(); index += 2U)
            {
            ...
            for (size_t index = 0; index < s_.size(); index += 2U)
            {
                encryptWords(left, right);
                s_[index] = left;
                s_[index + 1U] = right;
        ...
        [[nodiscard]] std::array<uint8_t, 8> decryptNativeBlock(
            const std::span<const uint8_t> encrypted) const
        {
            if (encrypted.size() != 8U)
                throw std::runtime_error("Blowfish block must contain exactly 8 bytes");

            auto left = detail::readLittleU32(encrypted.data());
            auto right = detail::readLittleU32(encrypted.data() + 4U);
            decryptWords(left, right);

            auto result = std::array<uint8_t, 8>{};
            detail::writeLittleU32(result.data(), left);
            detail::writeLittleU32(result.data() + 4U, right);
            return result;
        }

        [[nodiscard]] std::array<uint8_t, 8> encryptNativeBlock(
            const std::span<const uint8_t> plain) const
        {
            if (plain.size() != 8U)
                throw std::runtime_error("Blowfish block must contain exactly 8 bytes");

            auto left = detail::readLittleU32(plain.data());
            auto right = detail::readLittleU32(plain.data() + 4U);
            encryptWords(left, right);
    ...
    private:
        [[nodiscard]] uint32_t substitute(const uint32_t value) const noexcept
        {
            const auto a = static_cast<uint8_t>(value >> 24U);
            const auto b = static_cast<uint8_t>(value >> 16U);
            const auto c = static_cast<uint8_t>(value >> 8U);
            const auto d = static_cast<uint8_t>(value);
            return ((s_[a] + s_[256U + b]) ^ s_[512U + c]) + s_[768U + d];
        }

        void encryptWords(uint32_t& left, uint32_t& right) const noexcept
        {
            for (size_t round = 0; round < 16U; ++round)
            {
                left ^= p_[round];
                right ^= substitute(left);
                std::swap(left, right);
            }
        ...
        void decryptWords(uint32_t& left, uint32_t& right) const noexcept
        {
            for (size_t round = 17U; round > 1U; --round)
            {
                left ^= p_[round];
                right ^= substitute(left);
                std::swap(left, right);
        ...
        std::array<uint32_t, 1024> s_;
    };

    [[nodiscard]] inline bool selfTest()
    {
        constexpr auto key = std::array<uint8_t, 32>{
        ...
        constexpr auto encrypted = std::array<std::array<uint8_t, 8>, 5>{{
            {{ 0x00, 0x2d, 0xc3, 0x62, 0xd0, 0xa4, 0xcc, 0x40 }},
            {{ 0x0a, 0x62, 0xd4, 0xa4, 0x45, 0x6a, 0x91, 0x11 }},
            {{ 0xb7, 0xde, 0xa8, 0x50, 0xe4, 0x09, 0xef, 0x68 }},
            {{ 0xfd, 0x9f, 0xa7, 0xf9, 0xce, 0x89, 0x0e, 0x0c }},
            {{ 0x71, 0x59, 0xd8, 0xc3, 0xa8, 0xff, 0x84, 0x4b }}
        }};
        constexpr auto expected = std::array<std::array<uint8_t, 8>, 5>{{
            {{ 0x02, 0x00, 0x00, 0x00, 0x15, 0x01, 0x8e, 0x01 }},
            {{ 0x78, 0x9c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }},
            {{ 0x0f, 0x00, 0x00, 0x00, 0xc1, 0x05, 0xd8, 0x3a }},
            {{ 0xb3, 0x67, 0x60, 0x60, 0xe0, 0xe0, 0xe0, 0x60 }},
            {{ 0x80, 0x01, 0x00, 0x05, 0x08, 0x00, 0x58, 0x00 }}
        }};

        const auto cipher = Cipher(key);
        for (size_t block = 0; block < encrypted.size(); ++block)
        {
            if (cipher.decryptNativeBlock(encrypted[block]) != expected[block])
                return false;
        }
        return true;
    }
